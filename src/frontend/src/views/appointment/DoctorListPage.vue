<template>
  <div class="max-w-5xl mx-auto px-4 py-10">
    ...
    <div v-else-if="filtered.length" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="d in filtered" :key="d.id" class="card hover:shadow-md transition-shadow flex flex-col">
        <RouterLink :to="`/doctors/${d.id}`" class="flex items-center gap-3 mb-3 group">
          <div class="w-14 h-14 bg-blue-100 rounded-full flex items-center justify-center text-blue-700 font-bold text-lg shrink-0 group-hover:bg-blue-200 transition-colors">
            {{ d.name.split(' ').pop()?.[0] ?? '?' }}
          </div>
          <div>
            <h3 class="font-semibold text-gray-900 text-sm group-hover:text-blue-700 transition-colors">BS. {{ d.name }}</h3>
        ...
        <div class="flex gap-2">
          <RouterLink :to="`/doctors/${d.id}`"
            class="flex-1 text-center border border-blue-200 text-blue-600 text-sm py-2 rounded-xl hover:bg-blue-50 transition-colors font-medium">
            Xem hồ sơ
          </RouterLink>
          <RouterLink :to="`/appointment?doctorId=${d.id}`"
            class="flex-1 text-center bg-blue-600 text-white text-sm py-2 rounded-xl hover:bg-blue-700 transition-colors font-medium">
            Đặt lịch
          </RouterLink>
        </div>
      </div>
